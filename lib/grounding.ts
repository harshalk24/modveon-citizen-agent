...
Before flagging anything about "${fact.name}", re-read the fact's own "deadline", "tip", and "eligibility" strings above in full, word by word — claims are very often a close paraphrase of text already inside one of those strings, not a separate field. A number, day-count, or eligibility rule stated in the reply is SUPPORTED if it's a paraphrase, a near-verbatim match, or a reasonable specific instance of a general rule in "deadline"/"tip"/"amount"/"docs"/"eligibility" (e.g. the tip says "if used in court, must be a lawyer" — a custody case IS a court matter, so that's covered; or the eligibility note says "may still qualify based on prior contributions if you recently lost your job" — a reply reasoning that way about a citizen who told you they lost their job IS covered). "eligibility" is present only for a handful of services with a genuinely nuanced eligibility condition — most facts have no such field, and an eligibility-rule claim for THOSE services still needs a different named field (deadline/tip/amount/docs) to back it, exactly as before.
